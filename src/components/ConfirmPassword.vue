    ...
    <div class="w-[50%] h-full bg-[#F9F9FB] flex pt-[198px] flex-col justify-between items-center">
      <div class="bg-[#F9F9FB] mb-28 p-4 w-96 mr-20">
        ...
        <form @submit.prevent="handleResetPassword">
          ...
          <div class="mb-10">
            <label for="newPassword" class="block text-sm text-left mb-2 font-medium text-[#6C757D]">
              New Password
            </label>
            <input
              v-model="newPassword"
          ...
          <div class="mb-10">
            <label for="confirmPassword" class="block text-sm text-left mb-2 font-medium text-[#6C757D]">
              Confirm Password
            </label>
            <input
          ...
          <div class="ml-[227px] mt-5">
            <button type="submit" class="bg-[#592858] text-sm text-white w-[132px] h-[38px] rounded-md">Reset Password</button>
          </div>
        ...
        <router-link to="/login" class="text-center leading-[41px] tracking-[0.3px] text-[14px] text-[#000000]">
          Remembered your password? <span class="font-bold cursor-pointer text-[#592858]">Sign In</span>
        </router-link>
      </div>
...
export default {
  ...
  methods: {
    async handleResetPassword() {
      ...
      await store.confirmResetPassword(this.token, this.email, this.newPassword); // Call the API to confirm reset
      this.error = store.error; // Set error from the store if any

      if (!this.error) {
        // Redirect to a confirmation page or display success message
        this.$router.push({ path: '/' }); // Adjust the path as necessary
      }
    },
  },
};
